...
function main() {
    ...
    setTimeout(() => {
        document.querySelector("#loading-text").innerHTML = "加载字体和文件需要一定时间";
    }, 1000);

    const getDataState = document.querySelector("#loading").getAttribute("data-state");
    setTimeout(() => {
        if (Boolean(getDataState)) {
            document.querySelector("#loading-text").innerHTML = "原因：<br />1、当前网速可能较慢<br />2、远端服务器响应较慢";
        }
    }, 10000);
    ...
    function getHitokoto() {
        axios
            ...
            .then((res) => {
                ...
                document.querySelector("#menu-hitokoto-text").innerHTML = hitokoto;
                document.querySelector("#menu-text-source").innerHTML = textSource;
            })
            .catch((err) => {
                console.error(err);
            });
    ...
    // 设置移动端一言点击自动刷新
    const menuleftCard = document.querySelector(".menu-top-card");
    let menuleftHitCount = 0;
    menuleftCard.addEventListener("click", () => {
        menuleftHitCount++;
        // 判断leftHitCount是否大于5，如果是则不刷新
        if (menuleftHitCount > 2) {
            getPopup("手速过快，过一会再试试吧！");
            setTimeout(() => {
                menuleftHitCount = 0;
            }, 10000);
